...
for (i in 1:500){
  ...
  CI_slope_lower = b0 - qt(.975,18)*sqrt(S)*sqrt((1/20) + mean(x)*mean(x)/SXX)
  CI_slope_upper = b0 + qt(.975,18)*sqrt(S)*sqrt((1/20) + mean(x)*mean(x)/SXX)
  if(CI_lower<= 10 && CI_upper >= 10){
    in_CI = in_CI + 1
  }
  if(CI_slope_lower <= 50 && CI_slope_upper >= 50){
    in_CI_int = in_CI_int + 1
  }
